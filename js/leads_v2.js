...
async function filterLeads() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`HTTP ${res.status} failed: ${text}`);
    }
    ...
    try {
      ...
      window.lastFetchDebug = `URL: http://localhost:3000/api/leads?${params.toString()}<br/>Response: ${textResponse.substring(0, 150)}`;
    ...
    totalLeads = meta?.total || 0;
    
    try {
      renderLeadsTable();
    } catch (e) { throw new Error('Render table failed: ' + e.message); }
...
function sortLeads(field) {
  ...
  // Sorting can be done server-side in a future update,
  // For now, just sort the current page locally
...
function renderLeadsTable() {
  ...
  const end   = Math.min(start + PAGE_SIZE, totalLeads);
  ...
  tbody.innerHTML = filteredLeads.map(l => buildLeadRow(l)).join('');

  // Pagination info
  document.getElementById('pagination-info').textContent =
    `Showing ${start + 1}–${end} of ${totalLeads} lead${totalLeads !== 1 ? 's' : ''}`;

  renderPagination(totalLeads);
}
...
function goToPage(p) {
  const maxPages = Math.ceil(totalLeads / PAGE_SIZE);
  if (p < 1 || p > maxPages) return;
  currentPage = p;
  filterLeads(); // Fetch the new page from server
