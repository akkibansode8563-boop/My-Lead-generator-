...
function renderDashboard() {
  ...
  if (demoBanner) demoBanner.style.display = hasApiKey ? 'none' : 'block';
  document.getElementById('stat-total').textContent     = leads.length;
  document.getElementById('stat-qualified').textContent = leads.filter(l => ['qualified','won'].includes(l.status)).length;
    ...
    <tr>
      <td>
        <div class="company-cell">
          <div class="company-avatar">${(l.name || '?').charAt(0)}</div>
          <div>
            <div class="company-name">${esc(l.name)}</div>
